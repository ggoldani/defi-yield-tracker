...
export class PositionRepo {
  ...
  upsert(position: Omit<Position, 'id'>): number {
    const stmt = this.db.prepare(`
      ...
      ON CONFLICT(address_id, chain_id, pool_address) DO UPDATE SET
  ...
  findByPool(addressId: number, poolAddress: string, chainId: number): Position | undefined {
    const row = this.db
      .prepare('SELECT * FROM positions WHERE address_id = ? AND pool_address = ? AND chain_id = ?')
      .get(addressId, poolAddress.toLowerCase(), chainId) as Record<string, unknown> | undefined;
