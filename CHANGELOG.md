...
- **Task 5a — `current_value_usd` (spot MVP):** `estimatePositionValueUsd` (`src/analytics/positions.ts`) runs during `rebuildPositionsForAddressChain` for **active** rows → `PositionRepo.upsert`. **V2 (`v2_lp`):** Pair **`balanceOf` × pro-rata reserves / `totalSupply`** × **`PriceProvider.getCurrentPrice`** per leg; **gauge caveat** — staked LP off Pair `balanceOf` can **understate** until gauge support. **V3 / Slipstream (`v3_nft`):** NPM **`positions(tokenId)`** with **`liquidity > 0`** gate (no open liquidity → **0**, no spot calls); amounts = **indexed net wei** `(deposited − withdrawn)` × spot — **no `slot0` / tick math** in 5a; documented **~5–15%+** band vs tick-exact **5b**. **Inactive:** mark **0**; RPC errors → **`log.warn`**, **0**. `valuationAbis` + `tests/unit/positionsValuation.test.ts`; **`skipSpotValuation`** skips RPC in indexer unit tests.
