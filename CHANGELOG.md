...
- **Task 5a — `current_value_usd` (spot MVP, V2):** `estimatePositionValueUsd` (`src/analytics/positions.ts`) runs during `rebuildPositionsForAddressChain` for **active** rows → `PositionRepo.upsert`. **V2 (`v2_lp`):** Pair **`balanceOf` × pro-rata reserves / `totalSupply`** × **`PriceProvider.getCurrentPrice`** per leg; **gauge caveat** — staked LP off Pair `balanceOf` can **understate** until gauge support. **Inactive / RPC:** mark **0**; failures → **`log.warn`**, **0**. **`skipSpotValuation`** skips RPC in indexer unit tests.
- **Task 5b — `current_value_usd` (tick-accurate CL):** For **open** **`v3_nft`**, valuation uses Uniswap V3–style **`getSqrtRatioAtTick` + `getAmountsForLiquidity`** (`src/analytics/clLiquidityMath.ts`) with pool **`slot0.sqrtPriceX96`** and NPM **`positions`** (`liquidity`, ticks, **`tokensOwed0` / `tokensOwed1`**). **Uncollected fees** are included in the USD mark. **`liquidity === 0` → 0** USD (no fees-only mark). The **5a CL** shortcut (**indexed net × spot**) is **superseded** for active CL rows; **V2 (5a)** unchanged. Extended `valuationAbis` (pool **`slot0`**), `tests/unit/positionsValuation.test.ts`, `tests/unit/clLiquidityMath.test.ts`.
...
- `rebuildPositionsForAddressChain` now sets **`current_value_usd`** for active positions (**V2** Task 5a, **`v3_nft`** Task 5b); **`PositionRebuildDeps.skipSpotValuation`** skips RPC for unit tests.
